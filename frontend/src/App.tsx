...
function EmptyState() {
  return (
    <div className='empty'>
      ...
      <p>
        Pick a tool and its parameters on the left, then intercept. syn scores
        the action, shows the exact trigger it fired on, and writes the decision
        to the audit trail.
      </p>
...
export default function App() {
      ...
      .catch(() => {
        setTools([
          ...
          },
          {
            ...
            parameters: { query: { type: "string", description: "SQL" } },
          },
        ]);
        setLoadingTools(false);
      });
  ...
  return (
    <div
      className={`app${view === "bootstrap" ? " app--bootstrap" : ""}${isMobile && view === "console" ? " app--mobile-console" : ""}`}
    >
          ...
          <>
            <nav className='mobile-subnav'>
              {decision ? (
                <>
                  <button
                    className={mobileSubview === "receipt" ? "active" : ""}
                    onClick={() => setMobileSubview("receipt")}
                  >
                    Receipt
                  </button>
                  <button
                    className={mobileSubview === "timeline" ? "active" : ""}
                    onClick={() => setMobileSubview("timeline")}
                  >
                    Audit trail
                  </button>
                </>
                ...
                <>
                  <button
                    className={mobileSubview === "intercept" ? "active" : ""}
                    onClick={() => setMobileSubview("intercept")}
                  >
                    Intercept
                  </button>
                  <button
            ...
            <main
              className='output'
              style={mobileSubview === "timeline" ? { padding: 0 } : undefined}
            >
              {error && <div className='error-msg'>{error}</div>}
              {!decision && mobileSubview === "intercept" && (
                <p className='mobile-intro'>
                  syn scores each action against six risk factors and checks for
                  dangerous session patterns before approving, escalating, or
                  blocking.
                </p>
              )}
              {decision ? (
                mobileSubview === "receipt" ? (
