...
The AI Bootstrap reads tool schemas (via MCP `tools/list` introspection or manual JSON input). A context-rich prompt (hardcoded to fintech domain for the hackathon) is sent to the LLM, which returns structured JSON. The JSON is converted to nested YAML with the LLM's reasoning comments preserved as YAML comments. Before the output reaches the Bootstrap Review UI, it passes through the same schema validation used for all config files. After approval, the generated config is written to `policy_config.bootstrap.yaml` and merged at request time — the base `policy_config.yaml` is never overwritten.
...
Parse all three YAML configs and verify schema compliance. Additionally, after AI Bootstrap output is produced, run it through the same schema validation before surfacing to the Bootstrap Review UI. This prevents "looked fine in the table but YAML parser breaks" failures.
