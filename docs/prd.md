...
**Continuous auto-registration:** When an unknown tool is encountered during an intercept, the gateway blocks it AND automatically triggers bootstrap rule generation in the background. The proposed rules enter a SQLite-backed pending review queue — no rules go live without human approval. The Bootstrap Review UI exposes a Pending Approvals tab where each tool can be reviewed, edited, and approved or rejected individually with a YAML diff view. After approval, the generated config is written to `policy_config.bootstrap.yaml` and merged at request time — the base `policy_config.yaml` is never overwritten.
