...
def compute_weighted_score(factor_scores: FactorScores, config: dict) -> float:
    ...
    return (
        ...
        + factor_scores.anomaly * weights.get("anomaly", 0.15)
        + factor_scores.data_sensitivity * weights.get("data_sensitivity", 0.10)
...
def apply_weighted_decision(weighted_score: float, config: dict) -> Decision:
    ...
    block_min = thresholds.get("block_min", 70.0)
    escalate_max = thresholds.get("escalate_max", 70.0)
    escalate_min = thresholds.get("escalate_min", 40.0)
